                    ...
                    <div class="col-xs-12">
                        ...
                        <div>
                            <div id="sample-table-2_wrapper" class="dataTables_wrapper form-inline no-footer">
                                <div class="row">
                                    <div class="col-xs-6">
                                        <a href="/canmou/user/add">
                                            <button class="btn btn-info">添加管理员</button>
                                        </a>
                                    </div>
                                    <div class="col-xs-6">
                                    </div>
                                ...
                                <table id="sample-table-2" class="table table-striped table-bordered table-hover dataTable no-footer">
                                    ...
                                    <tbody>
                                    <?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr role="row" class="even">
                                            <td class="center">
                                                <label class="position-relative">
                                                    <input type="checkbox" class="ace">
                                                    <span class="lbl"></span>
                                            ...
                                            <td><?php echo (date("Y-m-d H:m:s",$user['created'])); ?></td>
                                            <td><?php echo (date("Y-m-d H:m:s",$user['last_login_time'])); ?></td>
                                            <td><?php echo ($user['last_login_ip']); ?></td>
                                            <td class="hidden-480 sorting_1">
                                                <?php if($user['status'] == 1): ?><span class="label label-sm label-info">启用</span>
                                                    <?php else: ?>
                                                    <span class="label label-sm label-warning">禁用</span><?php endif; ?>
                                            </td>
                                            <td>
                                                <div class="hidden-sm hidden-xs action-buttons">
                                                    <a class="green" href="<?php echo ($user['id']); ?>" >
                                                        <i class="ace-icon fa fa-pencil bigger-130"></i>
                                                    </a>
                                                    <a class="red" href="<?php echo ($user['id']); ?>" >
