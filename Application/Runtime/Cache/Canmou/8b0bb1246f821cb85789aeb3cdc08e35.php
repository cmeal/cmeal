                    ...
                    <div class="col-xs-12">
                        <form class="form-horizontal" role="form" method="POST">
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right" for="form-field-1">类目名称</label>
                                <div class="col-sm-6">
                                    <input type="text" id="form-field-1" class="col-xs-10 col-sm-6" name="name" value="<?php echo ($cate["name"]); ?>"/>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-left" for="form-field-1">描述
                                </label>
                                <div class="col-sm-6">
                                    <input type="text" id="form-field-1" class="col-xs-10 col-sm-6" name="desc" value="<?php echo ($cate["desc"]); ?>"/>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-left" for="form-field-1">类目</label>
                                <div class="col-sm-4">
                                    <select class="form-control" id="form-field-select-1" name="parent_id">
                                        <?php if($cate["parent_id"] != null AND $cate["parent_id"] != '0'): ?><option value="<?php echo ($cate["parent_id"]); ?>"><?php echo ($cate["parent_name"]); ?></option>
                                            <?php else: ?>
                                            <option value="0">顶级类目</option><?php endif; ?>
                                        <?php if(is_array($parent_cates)): $i = 0; $__LIST__ = $parent_cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ca): $mod = ($i % 2 );++$i; if(isset($cate) AND ($ca["cid"] == $cate.cid)): else: ?>
                                                <option value="<?php echo ($ca["cid"]); ?>"><?php echo ($ca["name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
